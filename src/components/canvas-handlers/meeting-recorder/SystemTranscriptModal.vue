      ...
      >
        ...
        >
          ...
          <div class="vdb-c-flex vdb-c-items-center vdb-c-gap-12">
            <!-- time pill (blue for system/visual distinction if desired, or keep orange) -->
            <span
              class="vdb-c-inline-flex vdb-c-items-center vdb-c-gap-4 vdb-c-rounded-[7px] vdb-c-bg-[#E6F0FF] vdb-c-px-8 vdb-c-py-4 vdb-c-text-[13px] vdb-c-font-[600] vdb-c-leading-[16px] vdb-c-text-black"
              style="color: #5095fb"
            >
...
const position = ref({ x: 400, y: 24 }); // Start slightly offset from Mic modal
...
watch(
  () => content.value?.system_transcript,
  (arr) => {
...
function itemKey(item, idx) {
  ...
  return `${item.speaker || "system"}-${item.start}-${idx}`;
}
</script>
